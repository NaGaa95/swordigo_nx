 ...
 *
 * Supports an arbitrary number of PT_LOAD segments, RELR/ANDROID_RELR packed
 * relocations and (for multi-module setups) cross-module symbol resolution.
 */
...
static uintptr_t so_resolve_symbol(so_module *mod, DynLibFunction *funcs, int num_funcs, const char *name) {
  // The import table takes priority; anything not in it falls through to the
  // exports of any other loaded module (used for multi-module setups).
